...
Dataset1_DE<-read.csv("November_Final__Full_metaOutputFDR_orderedByPval.csv", header=TRUE, stringsAsFactors = FALSE)

str(Dataset1_DE)

Dataset2_DE<-read.csv("Nontrad_metaOutputFDR_orderedByPval.csv", header=TRUE, stringsAsFactors = FALSE)
...
colnames(Dataset1_DE)[11]<-"GeneSymbol"
colnames(Dataset2_DE)[11]<-"GeneSymbol"
...
write.csv(Dataset1_vs_Dataset2, "Trad_vs_Nontrad.csv")
...
setwd(ComparisonforPhi)
pdf("Scatterplot_Dataset1_vs_Dataset2_Log2FC_AllGenes.pdf", height=5, width=5)
plot(Dataset2_Log2FC_estimate~Dataset1_Log2FC_estimate, data=Dataset1_vs_Dataset2, xlab="Traditional Log2FC", ylab="Non-traditional Log2FC")
TrendLine<-lm(Dataset2_Log2FC_estimate~Dataset1_Log2FC_estimate, data=Dataset1_vs_Dataset2) #Fitting a trendline
...
#Scatter plot with differentially expressed genes only at FDR < 0.05
library(dplyr)

TopGenes <- Dataset1_vs_Dataset2 %>%
  filter(Dataset1_FDR < 0.05 & Dataset2_FDR < 0.1)
pdf("Scatterplot_TopGenesBothDatasets_Log2FC.pdf", height=5, width=5)
plot(Dataset2_Log2FC_estimate ~ Dataset1_Log2FC_estimate,
     data = TopGenes,
     xlab = "HPC Log2FC (DEG)",
     ylab = "Cortex Log2FC (DEGs)",
     main = "Log2FC: Top Genes in Both Datasets")
TrendLine_Top <- lm(Dataset2_Log2FC_estimate ~ Dataset1_Log2FC_estimate, data = TopGenesBoth)
abline(TrendLine_Top, col="blue", lwd=2)
dev.off()

summary.lm(TrendLine_Top)
rm(TrendLine_Top)
...
cor.test(Dataset1_vs_Dataset2$Dataset2_Log2FC_estimate, Dataset1_vs_Dataset2$Dataset1_Log2FC_estimate, method="pearson")

#Calculating the Spearman rank correlation:
cor.test(Dataset1_vs_Dataset2$Dataset2_Log2FC_estimate, Dataset1_vs_Dataset2$Dataset1_Log2FC_estimate, method="spearman")
...
Dataset1_vs_Dataset2$Dataset1_Tstat<-(Dataset1_vs_Dataset2$Dataset1_Log2FC_estimate/Dataset1_vs_Dataset2$Dataset1_SE)

Dataset1_vs_Dataset2$Dataset2_Tstat<-(Dataset1_vs_Dataset2$Dataset2_Log2FC_estimate/Dataset1_vs_Dataset2$Dataset2_SE)
...
RRHO(list1, list2, labels=c("Dataset1 T-stat", "Dataset2 T-stat"), plots=TRUE, alternative="two.sided", outputdir="/Users/evageoghegan/ComparisonforPhi", BY=TRUE, log10.ind=TRUE)

#Making fgsea results for traditional meta-analysis output and combination of Phi's and Adrienne's results
...
DEResults<-read.csv("TraditionalMetaResults.csv", header=TRUE, stringsAsFactors = FALSE)
...
BrainGMT <- gmtPathways("Mouse_PhiNguyen_FLX_AsGeneSets_NoC5_wAdriennes.gmt.txt")
...
GSEA_Results<-fgsea(BrainGMT, DEResults_Log2FC_forGSEA_Ranked, nperm=10000, minSize = 10, maxSize = 2500)
#Warning messages:
...
write.csv(GSEA_Results, "Phi_Adrienne_data_traditional_fgsea.csv")
...
## Adding the ability to pull out DEGs from the leading edge 
#Wednesday 15th October 2025 Eva
setwd("/Users/evageoghegan/ComparisonforPhi")
fGSEA_Results<-read.csv("Phi_Adrienne_data_traditional_fgsea.csv", header=TRUE, stringsAsFactors = FALSE)
...
MetaFDR05_wValidInfo_Exploratory_fGSEA <- read.csv("HPC_trad_DEGS.csv", header = TRUE, stringsAsFactors = FALSE)



str(MetaFDR05_wValidInfo_Exploratory_fGSEA)
#'data.frame':	65 obs. of  1 variable:
#$ DEGS: chr  "Ebi3" "Rfc5" "Lrrc1" "Mad2l1" ...
...
write.csv(fGSEA_Sig, "Test_Trad_Phi_Adrienne_fgsea_matchedDEGs.csv")

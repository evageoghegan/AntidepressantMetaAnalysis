...
Dataset1_DE<-read.csv("HPC_metaOutputFDR_orderedByPval.csv", header=TRUE, stringsAsFactors = FALSE)

str(Dataset1_DE)

Dataset2_DE<-read.csv("PFC_metaOutputFDR_orderedByPval.csv", header=TRUE, stringsAsFactors = FALSE)
...
plot(Dataset2_Log2FC_estimate~Dataset1_Log2FC_estimate, data=Dataset1_vs_Dataset2, xlab="HPC Log2FC", ylab="Cortex Log2FC")
...
RRHO(list1, list2, labels=c("Dataset1 T-stat", "Dataset2 T-stat"), plots=TRUE, alternative="two.sided", outputdir="/Users/evageoghegan/Brain Data Alchemy Project", BY=TRUE, log10.ind=TRUE)
